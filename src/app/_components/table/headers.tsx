        ...
        <div {...props} className="flex items-center gap-4">
            <div className={"grid select-none flex-grow p-2 grid-cols-2 gap-4 *:text-sm " + className}>
                <div className="flex items-center">
                    <p 
                    onClick={sortingCallback && sortingCallback[0] && sortingCallback[0]} 
                    className={["text-sm text-neutral-500", sortingCallback && sortingCallback[0] && "hover:cursor-pointer"].join(' ')}>
                        Name
                    </p>
                    {sort && sort.type == 0 && (
                        <ChervonSVG
                ...
                <div
                    ...
                    {fields.map((field: string, id: number) => (
                        <div key={id} className={"lg:flex flex-row items-center hidden last:flex justify-center last:!justify-end"}>
                            <div className={["flex flex-row items-center", sortingCallback && sortingCallback[id + 1] && "hover:cursor-pointer"].join(' ')}>
                                <p
            ...
            {isEditable && (
                <div className="group !opacity-0 !select-none !pointer-events-none w-fit hidden lg:flex hover:cursor-pointer items-center panel py-1 px-3 duration-300 hover:bg-white/10">
