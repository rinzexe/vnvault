                        ...
                        <div className="w-full flex flex-col gap-4 items-center">
                            ...
                            <div className="grid gap-4 grid-cols-3 w-full">
                                ...
                                <div className="col-start-2 col-end-4">
                                    <p dangerouslySetInnerHTML={{ __html: formatDescription(vnData.description) }} className="text-sm"></p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="flex flex-col items-center w-full">
                                ...
                                <div className="block w-full">
                                    {vnData.screenshots.map((screenshot: any, id: number) => (
                                        screenshot.sexual < 1 && <Image width={500} height={400} className="inline-block p-2 w-3/6 h-auto" key={id} src={screenshot.url} alt="" />
                                    ))}
                                </div>
                            </div>
                        )}
...
function TagBadge({ name, category }: { name: string, category: string }) {
    if (category == "cont") {
        return <Badge color="#ffffff">{name}</Badge>
    }
    if (category == "tech") {
        return <Badge color="#ffffb5">{name}</Badge>
    }
    if (category == "ero") {
        return <Badge color="#ffb5e6">{name}</Badge>
    }
}
